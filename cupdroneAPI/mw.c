...
#define debugRegisted	6

int16_t debugReg[4];
void debugRegister(int8_t index, int16_t data)
{
	debugReg[index] = data;
}
...
void debugPrint(uint8_t debugMode, uint8_t counterMax)
{
	...
	case debugRegisted:
		...
		if(debugCounter > counterMax)
		{
			Hw_VCom_Printf("data0: %d, data1: %d, data2: %d, data3: %d", debugReg[0], debugReg[1], debugReg[2], debugReg[3]);
			debugCounter = 0;
		}
		break;
	}
...
static void pidMultiWii(void)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        */
        ...
        if(axis == ROLL)
        {
        	logRegister(1, -((int32_t)gyroData[axis] * dynP8[axis] / 10 / 8) - DTerm);
        }
        */
    }
}
...
/***************Flip Sensor Manage*****************/
typedef struct flipSensor_t
{
	int16_t angle[2];
	int16_t gyroRate;
} flipSensor_t;

flipSensor_t flipSens_t;

typedef struct flipSensor
{
...
void flipSensor_set(uint8_t axis, uint8_t dir)
{
	if(dir == clockWise)
	{
		flipSens_t.angle[signedType] = -angle[axis];

		flipSens_t.angle[unsignedType] = -angle[axis];
		if(flipSens_t.angle[unsignedType] < 0)
			flipSens_t.angle[unsignedType] = 3600 + flipSens_t.angle[unsignedType];

		flipSens_t.gyroRate = -gyroData[axis];
	}
	else
	{
		flipSens_t.angle[signedType] = angle[axis];
...
int16_t flipSensor_read(uint8_t target)
{
	int16_t returnVal = 0;
	switch(target)
	{
	case signedAngle:
		returnVal = flipSens_t.angle[signedType];
		break;
	case unsignedAngle:
		returnVal = flipSens_t.angle[unsignedType];
		break;
	case angularVelocity:
		returnVal = flipSens_t.gyroRate;
		break;
	}
...
flipSensor flipSens = {flipSensor_set, flipSensor_read};
...
typedef struct flipSchedulerSet
{
	uint8_t type;
	uint8_t currentStep;
	uint8_t nextStep;
...
flipSchedulerSet flipSchedulerState[] =
{
		//condition , current step  , next step		, target
		{timePass	, acceleration	, startRotate	, 143	}, //acceleration
		{anglePass	, startRotate	, stopRotate	, 1700	}, //startRotate
		{anglePass	, stopRotate	, recovery		, -100	}, //stopRotate
		{timePass	, recovery		, pidControl	, 143	}, //recovery

		{flipTrigger, pidControl	, acceleration	, 0		}  //pidControl
};
...
uint8_t anglePassed(uint8_t currentStep, uint8_t nextStep, int16_t targetAngle)
{
	int16_t currentAngle;
	currentAngle = flipSens.read(signedAngle);

	if(abs(currentAngle) > 450)
	{
		currentAngle = flipSens.read(unsignedAngle);
...
uint8_t flipStepScheduler()
{
	...
	switch(flipSchedulerState[step].type)
	{
	case flipTrigger:
		step = gotoFlip();
		break;
	case timePass:
		step = timePassed(flipSchedulerState[step].currentStep, flipSchedulerState[step].nextStep, flipSchedulerState[step].target);
		break;
	case anglePass:
		step = anglePassed(flipSchedulerState[step].currentStep, flipSchedulerState[step].nextStep, flipSchedulerState[step].target);
		break;
	}
...
{
		//ROLL	 PITCH	 YAW	 THROTTLE  PID Controller	THROTTLE_RAW
		{0		, 0		, 0		, 1850	, pidPrimary		, 1850		}, 	// acceleration
		{-250	, 0		, 0		, 1101	, pidGyrorate		, 1101		},	// startRotate
		{0		, 0		, 0		, 1850	, pidGyrorate		, 1850		},	// stopRotate
		{0		, 0		, 0		, 1850	, pidPrimary		, 1850		}   // recovery
};

int16_t gradualChange(uint8_t direction, int16_t angle_0, int16_t angle_f, int16_t angle, int16_t data_0, int16_t data_f)
{
	int16_t target;

	int16_t currentAngle;
	currentAngle = angle;
	if(direction == clockWise) currentAngle = -currentAngle;

	int16_t angle1;
	angle1 = angle_f - currentAngle;
	if(angle1 < 0) angle1 = 3600 + angle1;

	int16_t angle2;
	angle2 = angle_f - angle_0;
	if(angle2 < 0) angle2 = 3600 + angle2;

	target = data_f + (angle1*(data_0 - data_f) / angle2);
...
void gyrothrottleChange(uint8_t step)
{
	const int8_t direction = clockWise;

	int16_t targetGyrorate, targetThrottle;

	targetGyrorate = gradualChange(	direction							,
									flipSchedulerState[step - 1].target	,
									flipSchedulerState[step].target		,
									angle[ROLL]							,
									flipState[step-1].roll				,
									flipState[step].roll				);

	targetThrottle = gradualChange(	direction							,
									flipSchedulerState[step - 1].target	,
									flipSchedulerState[step].target		,
									angle[ROLL]							,
									flipState[step-1].throttle			,
									flipState[step].throttle			);

	rcCommand[ROLL] 	= targetGyrorate;
	rcCommand[PITCH]	= flipState[step].pitch;
	rcCommand[YAW] 		= flipState[step].yaw;
	rcCommand[THROTTLE] = targetThrottle;

	flipState[step].pid();
}

void throttleChange(uint8_t step)
{
	const int8_t direction = clockWise;
	...
	rcCommand[ROLL] 	= flipState[step].roll;
	rcCommand[PITCH]	= flipState[step].pitch;
	rcCommand[YAW] 		= flipState[step].yaw;
	rcCommand[THROTTLE] = targetThrottle;
...
void flipStateMachine(uint8_t step)
{
	...
	/*else if(step == startRotate)
	{
		throttleChange(step);
	}
	else if(step == stopRotate)
	{
		gyrothrottleChange(step);
	}*/
	else
	{
...
void logging(uint8_t step)
{
	...
	if(LOG.start)
	{
		...
		logRegister(0, gyroData[ROLL]>>2);
		logRegister(1, axisPID[ROLL]);
...
void loop(void)
{
    ...
    if (mcfg.looptime == 0 || (int32_t)(currentTime - loopTime) >= 0) {
        ...
        /*
        debugRegister(0, rcData[ROLL]);
        debugRegister(1, rcCommand[ROLL]);
        debugRegister(2, rcData[THROTTLE]);
        debugRegister(3, rcCommand[THROTTLE]);
        debugPrint(debugRegisted, 20);
        */
